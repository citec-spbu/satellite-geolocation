...
class pcpvt_small(CPVTV2):
    def __init__(self, pretrained=False, **kwargs):
        ...
        if pretrained:
            self.load_param(
                "/home/dmmm/VscodeProject/FPI/pretrain_model/pcpvt_small.pth"
            )
...
class PathAggregationNetwork(nn.Module):
    def __init__(self, in_channels_list, depth):
        ...
        for i, ch in enumerate(in_channels_list):
            self.inner_blocks.append(
                ConcatBlock(2 * ch if i < 2 else in_channels_list[-1], ch, depth, False)
            )
            if i > 0:
    ...
    def forward(self, x):
        ...
        for i in range(len(x) - 2, -1, -1):
            ...
            last_inner = self.inner_blocks[i](
                torch.cat((inner_top_down, x[i]), dim=1)
            )  # official
            # last_inner = self.inner_blocks[i](torch.cat((x[i], inner_top_down), dim=1)) # old
            results.insert(
                0, last_inner if i == 0 else self.layer_blocks[i - 1](last_inner)
            )
        ...
        for i in range(len(x) - 1):
            ...
            if i == 1:
                results[i + 1] = self.outer_blocks[i](
                    self.outconv1(torch.cat((outer_bottom_up, layer_result), dim=1))
                )
            else:
                results[i + 1] = self.outer_blocks[i](
                    torch.cat((outer_bottom_up, layer_result), dim=1)
                )
...
class ConcatBlock(nn.Module):
    def __init__(self, in_channels, out_channels, layer, fusion):
        ...
        self.part1 = nn.Sequential(
            ...
            nn.Sequential(
                *[FusionBlock(mid_channels, mid_channels, fusion) for _ in range(layer)]
            ),
