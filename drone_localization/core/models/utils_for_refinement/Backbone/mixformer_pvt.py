    ...
    def forward_features(self, template, search):
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                s_pos_embed = self._get_pos_embed(
                    pos_embed[:, 1:], patch_embed, s_H, s_W
                )
                t_pos_embed = self._get_pos_embed(
                    pos_embed[:, 1:], patch_embed, t_H, t_W
                )
            else:
...
def get_mixformer_pvt(vit_type, satellite_size=384, uav_size=128, **kwargs):
    ...
    if vit_type == "PvT-S":
        ...
        raise KeyError(
            f"VIT_TYPE shoule set to 'small_patch16' or 'large_patch16' or 'base_patch16'"
        )
