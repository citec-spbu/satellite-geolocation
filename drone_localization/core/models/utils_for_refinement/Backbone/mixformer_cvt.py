...
class Attention(nn.Module):
    ...
    def _build_projection(
        self, dim_in, dim_out, kernel_size, padding, stride, method, norm
    ):
        if method == "dw_bn":
            proj = nn.Sequential(
                OrderedDict(
    ...
    def forward_conv(self, x, t_h, t_w, s_h, s_w):
        ...
        template = rearrange(
            template, "b (h w) c -> b c h w", h=t_h, w=t_w
        ).contiguous()
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        ...
        q = rearrange(
            self.proj_q(q), "b t (h d) -> b h t d", h=self.num_heads
        ).contiguous()
        k = rearrange(
            self.proj_k(k), "b t (h d) -> b h t d", h=self.num_heads
        ).contiguous()
        v = rearrange(
            self.proj_v(v), "b t (h d) -> b h t d", h=self.num_heads
        ).contiguous()
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    def forward(self, template, search):
        ...
        for i in range(self.num_stages):
            ...
            res_template_list.append(template)
            res_search_list.append(search)

        return res_template_list, res_search_list
