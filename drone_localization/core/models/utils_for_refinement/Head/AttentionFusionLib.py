...
from .utils import ChannelPool, xcorr_depthwise, xcorr_fast, xcorr_slow
...
class AttentionFusionLib(nn.Module):
    def __init__(
        ...
        enable_position_embedding=False,
        pos_length=[128, 384],
        enable_gc=False,
        mid_ndim=2048,
        dropout_rate=0.1,
        ...
        **kwargs,
        ...
        decoderCFA_layer = DecoderCFALayer(
            input_ndim, nhead, mid_ndim, dropout_rate, activation
        )
        ...
        if enable_position_embedding:
            ...
            self.pos_embed_uav = nn.Parameter(
                torch.zeros(1, self.num_patches_uav, input_ndim), requires_grad=False
            )
    ...
    def init_pos_embed(self):
        ...
        pos_embed_uav = get_2d_sincos_pos_embed(
            ...
            int(self.num_patches_uav**0.5),
            cls_token=False,
        )
        self.pos_embed_uav.data.copy_(
            torch.from_numpy(pos_embed_uav).float().unsqueeze(0)
        )

        pos_embed_satellite = get_2d_sincos_pos_embed(
            self.pos_embed_satellite.shape[-1],
            int(self.num_patches_satellite**0.5),
            cls_token=False,
        )
    ...
    def forward_single(
        ...
        if self.fusion_type == "CAT_SA":
            ...
            src_search = torch.cat((src_temp, src_search), dim=1).permute(
                1, 0, 2
            )  # 16*(49+36)*384
            if self.enable_position_embedding:
                ...
                pos_search = torch.cat(
                    (self.pos_embed_uav, self.pos_embed_satellite), dim=1
                ).transpose(0, 1)
        ...
        memory_temp, memory_search = self.encoder(
            src1=src_temp,
            src2=src_search,
        ...
        if self.fusion_type == "CAT_SA":
            memory_temp, memory_search = torch.split(
                memory_search, [temp_out, search_out], dim=0
            )
        ...
        memory_search = memory_search.reshape(
            memory_search.shape[0], memory_search.shape[1], search_size, search_size
        ).contiguous()
...
class DecoderCFALayer(nn.Module):
    ...
    def forward_post(
        ...
        tgt2 = self.multihead_attn(
            query=self.with_pos_embed(tgt, pos_dec),
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward(
        ...
        return self.forward_post(
            ...
            src1_mask,
            src2_mask,
            src1_key_padding_mask,
            src2_key_padding_mask,
            pos_src1,
            pos_src2,
        )
