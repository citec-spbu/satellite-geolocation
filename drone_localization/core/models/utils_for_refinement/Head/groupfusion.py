...
class SingleGroupFusionHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.out_scale = (
            nn.Parameter(torch.tensor(out_scale), requires_grad=True)
            if scale_learnable
            else out_scale
        )
    ...
    def forward(self, z, x):
        ...
        if self.enable_reg:
            ...
            reg_r = (
                self.loc_adjust(xcorr_fast(z_reg, x_reg, padding=self.padding))
                * self.out_scale
            )
        return cls_r, reg_r
...
class DepthwiseFusion(nn.Module):
    def __init__(
        self, input_ndim=128, mid_ndim=128, padding=True, enable_reg=False, **kwargs
    ):
...
class MultiGroupFusionHead(nn.Module):
    def __init__(
        ...
        scale_learnable=True,
        out_scale=0.001,
        fusion_type="SingleGroupFusionHead",
        pool="avg",
        muti_level_nums=3,
        input_ndim=128,
        mid_ndim=128,
        ...
        **kwargs,
        ...
        for _ in range(muti_level_nums):
            if fusion_type == "SingleGroupFusionHead":
                single_head_func = SingleGroupFusionHead(
...
class MultiEnhanceGroupFusionHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pool = ChannelPool(mode=pool, linear_input_dim=muti_level_nums)
        self.merged_linear = nn.Conv2d(
            muti_level_nums * single_output_channel,
            merged_linear_channel,
            kernel_size=1,
            stride=1,
            padding=0,
        )
    ...
    def _fast_xcorr(self, z, x):
        ...
        z = (
            z.contiguous()
            .view(zn, gc_out_channel, self.gc_mid_channel, zh, zw)
            .contiguous()
            .view(zn * gc_out_channel, self.gc_mid_channel, zh, zw)
            .contiguous()
        )
