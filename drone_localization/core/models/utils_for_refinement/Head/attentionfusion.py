...
class AttentionFusionBlock(nn.Module):
    def __init__(self, opt) -> None:
        ...
        self.pos_embed_satellite = nn.Parameter(
            torch.zeros(1, self.num_patches_satellite, input_ndim), requires_grad=False
        )
    ...
    def forward_single(self, z, x):
        ...
        x_uav, x_satellite = torch.split(
            x, [H_uav * W_uav, H_satellite * W_satellite], dim=1
        )
        ...
        x_satellite_2d = x_satellite.transpose(1, 2).reshape(
            B, 1, H_satellite, W_satellite
        )
...
class CrossAttentionFusion(nn.Module):
    def __init__(self, opt):
        ...
        # norm module init
        self.NormModuleList = nn.ModuleList(
        ...
        for _ in range(linear_layer_num):
            self.LinearModuleList.append(
                nn.Sequential(
                    nn.Linear(input_ndim, mid_ndim), nn.GELU(), nn.Dropout(dropout_rate)
                )
            )
        self._reset_parameters()
