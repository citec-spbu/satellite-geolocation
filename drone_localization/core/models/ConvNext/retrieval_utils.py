...
def load_network(name, opt, weights_dir):
    ...
    if opt.views == 2:
        model = two_view_net(opt.nclasses, block=opt.block, return_f=False, resnet=opt.resnet)
    else:
        raise ValueError(f"Unknown views: {opt.views}")
    save_path = os.path.join(weights_dir, last_model_name)
