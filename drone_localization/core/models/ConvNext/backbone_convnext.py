...
class ConvNeXt(nn.Module):
    ...
    def forward_features(self, x):
        """Возвращает (gap_feature, spatial_features)"""
        for i in range(4):
            x = self.downsample_layers[i](x)
            x = self.stages[i](x)
        # Gap feature + spatial features (как в оригинале)
        return self.norm(x.mean([-2, -1])), x  # ← исправлено!

    def forward(self, x):
        gap, spatial = self.forward_features(x)
        return self.head(gap)


class LayerNorm(nn.Module):
    """LayerNorm с поддержкой channels_first и channels_last"""
...
def convnext_tiny(pretrained=True, in_22k=True):
    """Создание ConvNeXt-Tiny с опциональной загрузкой весов"""
    ...
    if pretrained:
        ...
        # Убираем head из предобученных весов (у нас свой classifier)
        checkpoint_model = {
            k: v for k, v in checkpoint["model"].items() if not k.startswith("head")
        }
